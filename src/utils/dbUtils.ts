...
export interface StoredPDF {
    ...
    canvasStates?: Record<number, Record<string, unknown>>; // Fabric.js canvas JSON for each page
}
...
export const updatePDFInDB = async (id: number, editedData: ArrayBuffer, canvasStates?: Record<number, Record<string, unknown>>): Promise<void> => {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const transaction = db.transaction([STORE_NAME], 'readwrite');
        const store = transaction.objectStore(STORE_NAME);
